...
export default withLayout(function Page() {
      ...
      for (const json of chunks) {
        ...
        try {
          ...
          if (json.length !== res?.itemCount) {
            logger.error(
              `Sent ${json.length} items and ${res?.itemCount ?? 'none'} passed validation`
            )
  ...
  return withAuth(
    <>
      ...
      <ul>
        <li>
          Spreadsheets must contain one item per row with an <code>id</code> column defined for
          every entry.
        </li>
        <li>
          JSON formatted files must contain an array of objects with at least the <code>id</code>{' '}
          key defined, for ex:<code>{`[{id:'id1', ...}, {id: 'id2', ...}, ...]`}</code>
        </li>
        <li>
